...
登录后，卡片标题停顿约 700ms 自动匹配候选，也可以打开“搜索游戏图片”单独修改关键词。选定游戏后浏览截图、宣传图或封面，预览并点击“使用此图”后才替换当前卡片图片。图片和来源记录存入草稿，可随项目保存到云端；不修改标题、日期和平台。中文名称未收录时可使用英文名或别名。

客户端仅访问已有私有 API，Twitch Client ID/Secret 与访问令牌均保留在 VPS。CDN 缩略图不携带会话 Cookie。与相册选图共用过期请求保护，关闭选图、切换卡片或图片被替换后，旧请求不会写回。

本次新增 8 项原生接口/来源测试与 1 项原生选图 UI 流程；iPhone Air 模拟器共 31 项单元测试、3 条 UI 流程通过。网站/服务器共 62 项测试通过，真实 Twitch/IGDB 中文与英文搜索、图库、图片下载、网页裁剪及重新载入已验证。

DEBUG 可加 `--demo --igdb-demo` 运行完全本地的 IGDB 模拟服务，覆盖自动候选→图库→预览取消→确认导入→重启保留；模拟图片地址由 CDN 校验拒绝，不发生外网请求。正式使用不要添加演示参数。
